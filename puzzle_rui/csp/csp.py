...
def min_conflicts(csp, max_steps=1000000): 
    print "Solving a CSP by min_conflicts....\n"
    # Generate a complete assignement for all vars (probably with conflicts)


    current = {}; csp.current = current
    ### MODIFIED ###
    # Do this instead by taking the first item from each list
    # Since they are sorted according to rank
    # current = {k:v[0] for k,v in csp.domains.items()}; csp.current = current
...
#        sys.stderr.write(str(i)+" ")
        ...
        if not conflicted:
#            sys.stderr.write("Found solution\n")
            return current,conflicted
        var = random.choice(conflicted)
        val = min_conflicts_value(csp, var, current)
        csp.assign(var, val, current)
    ### MODIFIED ###
#    sys.stderr.write("Done iterating\n")
    return current,conflicted
